 ...
 */
export function extractLocalImages(markdown: string, basePath?: string): ImageReference[] {
  const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
  ...
  while ((match = imageRegex.exec(markdown)) !== null) {
    ...
    if (isLocal) {
      // 相対パスを絶対パスに変換
      const absolutePath = basePath ? path.resolve(basePath, imagePath) : path.resolve(imagePath);
 ...
 */
export function replaceImagePaths(markdown: string, replacements: Map<string, string>): string {
  let result = markdown;
 ...
 */
export function processMarkdown(markdown: string, basePath?: string): ProcessedMarkdown {
